...
private:
    ...
    std::vector<Transaction>::iterator transactionsBinarySearch(AccountsMap::iterator accountIt, unsigned int txNo);
    ...
    bool isTransactionDuplicate(const Transaction& transaction, AccountsMap::iterator accountIt);

    void sortTransactionsData();
    void calculateAverageOfTransactions();
};
